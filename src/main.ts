...
import type { BranchUpdate, DirectoryContext, OverrideEntry, UpdateCandidate, VersionReleaseNote } from './types'
...
function parseArgs(): { dryRun: boolean; configPath: string; excludeDirectories: string[] } {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (arg === '--help' || arg === '-h') {
      ...
      i++
    } else if ((arg === '--exclude' || arg === '-e') && args[i + 1] !== undefined) {
...
function buildDirectoryContext({ cwd, workingDirectory }: { cwd: string; workingDirectory: string }): DirectoryContext {
  ...
  return {
    ...
    packageJsonRelPath: isRoot ? 'package.json' : `${workingDirectory}/package.json`
  }
}
...
async function processDirectory({
  ...
  const config = await loadConfig({ configPath: `${dir.workDir}/${configPath}` })
  ...
  const effectiveBranchPrefix = workingDirectory === '.'
    ? config.branchPrefix
    : `${config.branchPrefix}/${workingDirectory}`
  const overrideBranchPrefix = getOverrideBranchPrefix({ branchPrefix: effectiveBranchPrefix })
  ...
  const existingPrs = await getExistingPrs({ cwd: dir.cwd, branchPrefix: effectiveBranchPrefix })
  console.log(`    Found ${existingPrs.length} existing catalog-update PRs`)

  const catalogPrs = existingPrs.filter((pr) => pr.headRefName.startsWith(`${effectiveBranchPrefix}/`))
  ...
  const catalogSyncResult = await syncExistingPrs({
    ...
    resolveBranchUpdate: (branchName: string) => {
      ...
      return buildCatalogBranchUpdate({ groupName, updates, config, titleSuffix, branchPrefix: effectiveBranchPrefix, releaseNotes })
    },
  ...
  if (overrideBranchUpdate && availableSlots > 0 && !existingBranches.has(overrideBranchUpdate.branch)) {
    const success = await createPr({ branchUpdate: overrideBranchUpdate, config, dir })
    if (success) {
      created++
      openPrCount++
      availableSlots--
  ...
  // Catalog PRs
  const skippedGroups = [...groups.keys()].filter((name) => existingBranches.has(`${effectiveBranchPrefix}/${name}`))
  ...
  for (const [groupName, updates] of groups) {
    ...
    const branchUpdate = buildCatalogBranchUpdate({ groupName, updates, config, titleSuffix, branchPrefix: effectiveBranchPrefix, releaseNotes })
...
async function main(): Promise<void> {
  ...
  if (excludeDirectories.length > 0) console.log(`Exclude: ${excludeDirectories.join(', ')}`)
  console.log('')

  // 0. Fetch latest remote refs
  ...
  const directories = await discoverCatalogDirectories({ cwd, excludePatterns: excludeDirectories })
  ...
  console.log(`Found ${directories.length} catalog ${directories.length === 1 ? 'directory' : 'directories'}: ${directories.map((d) => d === '.' ? '.' : `/${d}`).join(', ')}`)
  ...
  for (const dir of directories) {
    ...
    try {
      const result = await processDirectory({ workingDirectory: dir, cwd, configPath, dryRun })
  ...
  if (!dryRun) {
    ...
    console.log(`\n${'='.repeat(60)}`)
    console.log(`Summary: Created ${totalCreated}/${total} PRs, rebuilt ${totalRebuilt} existing PRs across ${directories.length} ${directories.length === 1 ? 'directory' : 'directories'}.`)
  }
